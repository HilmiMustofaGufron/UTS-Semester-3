...
public class Dashboard extends JFrame {
    ...
    private JPanel createAddCarPanel() {
        ...
        saveCarButton.addActionListener(e -> {
            ...
            try {
                ...
                carManager.addCar(new Car(plate, type, color, pricePerDay));
                updateCarDropdown();
    ...
    private JPanel createRentCarPanel() {
        ...
        calculateButton.addActionListener(e -> {
            ...
            if (selectedIndex >= 0) {
                try {
                    ...
                    int pricePerDay = carManager.getCars().get(selectedIndex).getPricePerDay();
        ...
        rentButton.addActionListener(e -> {
            ...
            try {
                ...
                int selectedIndex = carDropdown.getSelectedIndex();
                if (selectedIndex >= 0 && selectedIndex < carManager.getCars().size()) {
                    Car car = carManager.getCars().get(selectedIndex);
                    if (!car.isRented()) {
                        car.setRented(true);
                        rentalManager.addRental(new Rental(car.getPlateNumber(), car.getType(), car.getColor(), customerName,
                                car.getPricePerDay(), address, phone, rentalDays));
                        JOptionPane.showMessageDialog(Dashboard.this, "Mobil berhasil disewa!\nTotal Harga: " + (rentalDays * car.getPricePerDay()));
    ...
    private JPanel createAvailableCarsPanel() {
        ...
        editButton.addActionListener(e -> {
            ...
            if (selectedRow >= 0) {
                ...
                Car carToEdit = carManager.findCarByPlateNumber(plateNumber);
                if (carToEdit != null) {
                    ...
                    JTextField priceField = new JTextField(String.valueOf(carToEdit.getPricePerDay()));
                    ...
                    if (result == JOptionPane.OK_OPTION) {
                        ...
                        carToEdit.setPricePerDay(Integer.parseInt(priceField.getText()));                        updateAvailableCarsTable();
                        updateCarDropdown();
                    }
                }
            } else {
        ...
        deleteButton.addActionListener(e -> {
            ...
            if (selectedRow >= 0) {
                String plateNumber = (String) availableCarsTableModel.getValueAt(selectedRow, 0);
                int confirm = JOptionPane.showConfirmDialog(Dashboard.this, "Are you sure you want to delete this car?", "Confirmation", JOptionPane.YES_NO_OPTION);
                if (confirm == JOptionPane.YES_OPTION) {
                    Car carToDelete = carManager.findCarByPlateNumber(plateNumber);
                    if (carToDelete != null) {
                        carManager.removeCar(carToDelete);
                        updateAvailableCarsTable();
                        updateCarDropdown();
                        JOptionPane.showMessageDialog(Dashboard.this, "Car successfully deleted!");
                    }
                }
            } else {
                JOptionPane.showMessageDialog(Dashboard.this, "Please select a car first.");
            }
        });
    ...
    private JPanel createRentedCarsPanel() {
        ...
        rentedCarsTableModel = new DefaultTableModel(new Object[]{"Plate Number", "Type", "Renter", "Rental Days", "Total Price"}, 0);
            ...
            if (selectedRow >= 0) {
                ...
                for (Rental rental : rentalManager.getRentals()) {
                    if (rental.getPlateNumber().equals(plateNumber)) {
                        ...
                        carManager.updateCarStatus(plateNumber, false);
                        JOptionPane.showMessageDialog(Dashboard.this, "Car successfully returned!");
                        updateRentedCarsTable();
                        updateCarDropdown();
                        break;
                    }
    ...
    private void updateCarDropdown() {
        ...
        if (carDropdown.getItemCount() == 0) {
            carDropdown.addItem("No cars available");
        }
    }
